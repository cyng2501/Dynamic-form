...
function formdatatable() {
    var datatable = document.getElementById('datatable');
    var y = datatable.rows.length;
    if (y == 0) {
        var heading = document.createElement("tr");
        heading.id = "datahead";
        datatable.appendChild(heading);
        var datahead = document.getElementById('datahead');
        var idhead = document.createElement("td");
        idhead.appendChild(document.createTextNode("Id"));
        datahead.appendChild(idhead);
        for (i = 0; i < fields.length; i++) {
            var dataheading = document.createElement("td");
            dataheading.appendChild(document.createTextNode(fields[i]));
            datahead.appendChild(dataheading);
        }
        var edithead = document.createElement("td");
        var deletehead = document.createElement("td");
        edithead.appendChild(document.createTextNode("Edit"));
        deletehead.appendChild(document.createTextNode("Delete"));
        datahead.appendChild(edithead);
        datahead.appendChild(deletehead);
        ...
        for (j = c; j < x; j++) {
            for (i = 0; i < y; i++) {
                if (i == 0) {
                    var datahead = document.getElementById('datahead');
                    var newdata = datahead.insertCell(datahead.cells.length - 2);
                    newdata.appendChild(document.createTextNode(fields[j]));
                } else {
                    var datarow = datatable.rows[i];
                    var newdata = datarow.insertCell(datarow.cells.length - 2);
                    newdata.appendChild(document.createTextNode("null"));
                }
            }
        }
    }
